"""Tests for the adapted retrieval functions.
...
_spec_retrieval = importlib.util.spec_from_file_location(
    "w3.retrieval",
    _PROJECT_ROOT / "w3" / "retrieval.py",
)
_retrieval_module = importlib.util.module_from_spec(_spec_retrieval)
sys.modules[_spec_retrieval.name] = _retrieval_module
_spec_retrieval.loader.exec_module(_retrieval_module)
...
sys.modules[_spec_store.name] = _store_module
...
class TestFilterByMetadata:
    def test_filter_by_exact_title(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        results = filter_by_metadata(
            "title", ["Taylor Swift wins Grammy"],
            store, collection_name, limit=5,
        )
        assert len(results) >= 1
        assert results[0]["title"] == "Taylor Swift wins Grammy"

    def test_filter_returns_multiple(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        results = filter_by_metadata(
            "title",
    ...
    def test_filter_no_match(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        results = filter_by_metadata(
    ...
    def test_filter_respects_limit(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        results = filter_by_metadata(
            "title",
            ["Taylor Swift wins Grammy", "US economic growth report", "Brazil signs trade deal with France"],
            store, collection_name, limit=2,
        )
        assert len(results) <= 2

    def test_filter_returns_dicts_with_required_fields(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        results = filter_by_metadata(
            "title", ["Taylor Swift wins Grammy"],
            store, collection_name, limit=5,
...
class TestSemanticSearchRetrieve:
    def test_returns_list_of_dicts(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        results = semantic_search_retrieve(
            "music", store, collection_name, _stub_embed, top_k=2,
        )
        assert isinstance(results, list)
        assert all(isinstance(r, dict) for r in results)

    def test_respects_top_k(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        for k in (1, 2, 3):
            results = semantic_search_retrieve(
                "test", store, collection_name, _stub_embed, top_k=k,
            )
            assert len(results) == k, f"Expected {k}, got {len(results)}"

    def test_required_fields_present(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        results = semantic_search_retrieve(
            "test", store, collection_name, _stub_embed, top_k=1,
        )
        for field in ("id", "title", "chunk", "pubDate", "link"):
            assert field in results[0]
    ...
    def test_empty_collection_returns_empty(self, store, collection_name):
        ...
        results = semantic_search_retrieve(
            "test", store, collection_name, _stub_embed, top_k=5,
        )
        assert results == []
...
class TestBm25Retrieve:
    ...
    def test_keyword_relevance(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
...
class TestHybridRetrieve:
    def test_returns_list_of_dicts(self, store, collection_name, sample_documents, sample_embeddings):
        ...
        results = hybrid_retrieve(
            "economy", store, collection_name, _stub_embed, alpha=0.5, top_k=2,
        )
        assert isinstance(results, list)
        assert all(isinstance(r, dict) for r in results)
    ...
    def test_respects_top_k(self, store, collection_name, sample_documents, sample_embeddings):
        ...
        for k in (1, 2, 3):
            results = hybrid_retrieve(
                "test", store, collection_name, _stub_embed, alpha=0.5, top_k=k,
            )
            assert len(results) == k

    def test_alpha_zero_is_bm25(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        bm25_results = bm25_retrieve("economy", store, collection_name, top_k=3)
        hybrid_results = hybrid_retrieve(
            "economy", store, collection_name, _stub_embed, alpha=0.0, top_k=3,
        )
        # With alpha=0.0, hybrid should match BM25 order
        assert len(bm25_results) == len(hybrid_results)

    def test_empty_collection(self, store, collection_name):
        store.create_collection(collection_name)
        results = hybrid_retrieve(
            "test", store, collection_name, _stub_embed, alpha=0.5, top_k=5,
        )
        assert results == []
    ...
    def test_required_fields_present(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        results = hybrid_retrieve(
            "test", store, collection_name, _stub_embed, alpha=0.5, top_k=1,
        )
        for field in ("id", "title", "chunk", "pubDate", "link"):
            assert field in results[0]
...
class TestSemanticSearchWithReranking:
    ...
    def test_rerank_by_chunk(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        # Query about "Grammy" should make doc1 rank higher when reranking by chunk
        results = semantic_search_with_reranking(
            "Grammy awards ceremony", "chunk",
            store, collection_name, _stub_embed, top_k=3,
        )
        assert len(results) >= 1
        assert results[0]["title"] == "Taylor Swift wins Grammy"

    def test_rerank_query_differs(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        # Use a different rerank query
        results = semantic_search_with_reranking(
            "music", "title",
            store, collection_name, _stub_embed,
            rerank_query="trade agreement", top_k=3,
        )
        assert len(results) >= 1
...
class TestGenerateFinalPrompt:
    ...
    def test_rag_formats_documents(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        result = generate_final_prompt(
        ...
        assert "economic" in result.lower() or "us economy" in result.lower()

    def test_prompt_contains_query(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        result = generate_final_prompt(
...
class TestLlmCall:
    def test_returns_prompt_when_no_backend(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
        result = llm_call(
            "test query",
            retrieve_function=semantic_search_retrieve,
    ...
    def test_uses_llm_backend(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
    ...
    def test_no_rag_passes_through(self, store, collection_name, sample_documents, sample_embeddings):
        _populate(store, collection_name, sample_documents, sample_embeddings)
