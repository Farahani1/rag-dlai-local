"""Tests for the W4 ChromaStore adapter.
...
# ``w4`` package by the ``tests.w4`` test package.
...
class TestAddDocuments:
    ...
    def test_add_to_nonexistent_collection_raises(
        self, store, sample_documents, sample_embeddings
    ):
...
class TestQuery:
    ...
    def test_query_returns_only_dicts(
        self, store, sample_documents, sample_embeddings
    ):
...
class TestQueryWithFilter:
    ...
    def test_filter_no_match(
        ...
        results = store.query_with_filter(
            ...
            query_emb,
            top_k=5,
            metadata_filter={"title": {"$in": ["Nonexistent"]}},
        )
        assert results == []
...
class TestUtilityMethods:
    ...
    def test_count(self, store, sample_documents, sample_embeddings):
        name = _collection_name()
        store.create_collection(name)
