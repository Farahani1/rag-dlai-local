...
Unlike ``w4/chroma_store.py`` (which hardcodes a ``title/chunk/pubDate/link``
metadata schema for news articles), this W5 version stores and returns
**arbitrary** metadata dicts, since W5 needs real product fields (``gender``,
``baseColour``, ``masterCategory``, ...) for filtering, and FAQ documents need
``question``/``answer``/``type``. Metadata values must still be Chroma-compatible
scalars (``str``, ``int``, ``float``, or ``bool``).
"""
...
class ChromaStore:
    ...
    def add_documents(
        ...
        documents :
            Each dict must contain an ``id`` key. Every other key/value pair
            is stored verbatim as Chroma metadata, so values must be
        ...
        metadatas = [
            {k: v for k, v in doc.items() if k != "id"} for doc in documents
        ]
    ...
    def get_all_documents(self, collection_name: str) -> list[dict[str, Any]]:
        ...
        -------
        list[dict]
            Each dict contains ``id`` plus every stored metadata field.
            Returns an empty list for an empty collection.
        """
        try:
            collection = self._client.get_collection(collection_name)
        except NotFoundError as exc:
